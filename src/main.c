...
/*   Updated: 2025/02/24 18:13:25 by cedmarti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	creation_cmds_dur(t_shell *shell)
{
	...
	shell->cmds[0].in = malloc(sizeof(t_redirection) * 1);
	shell->cmds[0].in[0].file = ft_strdup("toto");
	shell->cmds[0].in[0].type = 2;
	// shell->cmds[0].in[1].file = ft_strdup("file2.txt");
	// shell->cmds[0].in[1].type = 1;

	shell->cmds[0].out = NULL;
	shell->cmds[0].count_out = 0;
	// shell->cmds[0].out = ft_calloc(2, sizeof(t_redirection));
	// shell->cmds[0].out[0].file = ft_strdup("outfile1.txt");
	// shell->cmds[0].out[0].type = 3;
	// shell->cmds[0].out[1].file = ft_strdup("file2.txt");
